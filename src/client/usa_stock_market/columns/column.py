...
class Column:
    def __init__(self, tech_name: str, description: str, allowedFor: List[ColType], output: str = None, needPercentConvert: bool = False) -> None:
        if len(allowedFor) == 0:
        ...
        self.__output = output if output is not None else tech_name.capitalize()
    ...
    def to_dict(self) -> Dict[str, any]:
        return {Column.OUTPUT_NAME_DICT_KEY: self.output_name,
    ...
    def from_dict(dict: Dict[str, any]) -> Column:
        return Column(dict[Column.TECH_NAME_DICT_KEY], dict[Column.DESCRIPTION_DICT_KEY], dict[Column.ALLOWED_FOR_ENUM], dict[Column.OUTPUT_NAME_DICT_KEY], dict[Column.NEED_PERCENT_CONVERT_DICT_KEY])
