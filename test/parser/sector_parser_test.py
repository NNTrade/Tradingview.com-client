...
from src.api.usa_stock_market import get_sectors_payload
...
class ParseHeader_TestCase(unittest.TestCase):
  ...
  def test_WHEN_request_THEN_correct_header_parsed(self):
    # Array
    asserted_body = get_sectors_payload()
    # Act
    asserted_header = parse_header_from_payload(asserted_body)
    # Assert
    for expected_header in self.expected_headers:
      self.assertTrue(expected_header in asserted_header)
